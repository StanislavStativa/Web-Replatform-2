...
import { event, IS_PROUSER, ProductList_PAYLOAD_CONTENT, PRODUCTLIST_SORT } from '@/config';
import { ClickType } from '../../Modal/modal.type';
import { DeviceType, useDeviceType } from '@/hooks/useDeviceType';

type ITypesProductCollectionCarousel = {
  SectionTitle: {
    jsonValue: TextField;
  };
...
const ProductCollectionCarousel = ({
  ...
  ButtonText,
  DiscoverRfkId,
}: ITypesProductCollectionCarousel) => {
  ...
  const { data } = useQuery({
    queryKey: ['productCarouselCollection'],
      ...
      buttonChild = item?.sample_sku_pro ? (
        <>
          {checkItemPresent(isSampleButton)
            ? t('Tag_SampleInCart')
            : ButtonFunctionality?.jsonValue?.value === BTNFUNCTION?.ADDSAMPLE
      ...
      if (typeof window !== 'undefined' && 'rfk' in window) {
        window.rfk.push({
          value: {
            ...
            rfkid: DiscoverRfkId?.jsonValue?.value,
            f: '',
            click_type: 'product',
  ...
  return (
    <div className="component container mx-auto md:px-10">
      {SectionTitle?.jsonValue?.value !== '' && dataResult?.length > 0 && (
        <Text
          field={SectionTitle?.jsonValue}
          tag={'h2'}
          className={cn('mt-10 mb-6 font-normal text-32 leading-10 text-center')}
        />
      )}
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex flex-col justify-center items-center gap-6 self-stretch text-dark-gray hover:text-black">
                          ...
                          {ButtonFunctionality?.jsonValue?.value === BTNFUNCTION?.SHOPNOW ? (
                            <Button
...
export default memo(ProductCollectionCarousel);
