...
const PDPFinishVariant: React.FC<ProductAttributesProps> = (data) => {
  ...
  const variantFinish = data?.data?.Finish || [];

  useEffect(() => {
    // Only initialize the carousel to the active slide on the initial load
    const activeIndex = variantFinish.findIndex((item) => item.IsActive);
    ...
    if (variantFinish?.length <= 2) {
      setIsAtStart(false);
      setIsAtEnd(false);
    } else if (
      carouselRef.current &&
      carouselRef.current.state.currentSlide === variantFinish?.length - 2
    ) {
      setIsAtEnd(false);
    }
  }, [variantFinish]);
      ...
      {variantFinish.length > 0 && (
        <div className="mt-10 variant-shapeandsize w-full relative">
          ...
          <div className="relative">
            ...
            >
              {variantFinish.map((card, index) => (
                <div key={index} className="px-2 relative">
                  <button
                    onClick={() => handleButtonClick(card?.ProductUrl)}
                    className={cn(
                      'max-w-2000 mb-1.5 dark-gray bg-white border-dark-gray text-center w-full border inline-block no-underline font-normal rounded-lg py-3 px-6 text-sm transition duration-100 relative',
                      card.IsActive
...
export default PDPFinishVariant;
