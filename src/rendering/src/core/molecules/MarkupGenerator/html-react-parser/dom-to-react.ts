...
export default function domToReact(
  ...
  const { cloneElement, createElement, isValidElement } = options.library || React;
  ...
  for (let index = 0; index < nodesLength; index++) {
    ...
    if (node.type === 'text') {
      ...
      // so skip it
      if (isWhitespace && node.parent && !canTextBeChildOfNode(node.parent as Element)) {
        continue;
      }
        ...
        if (element.attribs.src) {
          ...
          reactElements.push(
            createElement(Script, {
              key: `custom-script-src-${index}`,
              src: element.attribs.src,
              strategy: 'beforeInteractive', // Adjust strategy based on requirement
            })
          );
          ...
          reactElements.push(
            createElement(Script, {
              ...
              strategy: 'afterInteractive', // Adjust strategy based on requirement
    ...
    // https://react.dev/learn/rendering-lists#rules-of-keys
    if (nodesLength > 1) {
      props.key = `${node.type}_${index}`;
    }

    reactElements.push(transform(createElement(node.name, props, children), node, index));
  }

  return (reactElements.length === 1 ? reactElements[0] : reactElements) as JSX.Element;
}
...
function skipAttributesToProps(node: Element): boolean {
  return (
    PRESERVE_CUSTOM_ATTRIBUTES && node.type === 'tag' && isCustomComponent(node.name, node.attribs)
  );
}
